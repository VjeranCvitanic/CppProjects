...
Card HumanPlayer::PlayCard(std::vector<Card> playedHand)
{
    if(hand.empty())
    {
    ...
    hand.erase(std::remove(hand.begin(), hand.end(), playedCard), hand.end());
    return playedCard;
}
...
void HumanPlayer::startGame()
{
    ...
    {
        print("Strong color: ");
        print(Cards::ColorToString(static_cast<Briscola*>(gamePtr)->getStrongColor()));
        newLine();
        print("Last card: ");
        print(Cards::CardToString(static_cast<Briscola*>(gamePtr)->getLastCard()));
        newLine();
    }
    printLines();
}
